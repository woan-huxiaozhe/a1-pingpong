...
def ensure_ht_runtime_buffers(env):
    ...
    env._ht_acc_ne_rate_sum = torch.zeros((), device=device)  # sum |dn/dt|=|omega x n| at hit [rad/s]
...
def pop_hittrack_tracking_stats(env):
    ...
    stats = {
        ...
        "normal_rate_at_hit": float(env._ht_acc_ne_rate_sum.item()) * inv,  # |dn/dt| [rad/s]; ~1 good, ~3.3 tumbling
    ...
    for buf in (env._ht_acc_n, env._ht_acc_success, env._ht_acc_pe_sum,
                ...
                env._ht_acc_ne_deg_sum, env._ht_acc_ne_dot_sum, env._ht_acc_ne_rate_sum):
        buf.zero_()
    return stats
...
def update_hit_track_state(
    ...
    *,
    ...
    if bool(at_hit.any()):
        ...
        racket_w = racket_ang_vel(env, RACKET_BODY_NAME)
        face_rate = torch.norm(torch.cross(racket_w, racket_n, dim=-1), dim=-1)  # |dn/dt|=|omega x n| [rad/s]
        ...
        env._ht_acc_ne_rate_sum += face_rate[at_hit].sum()
