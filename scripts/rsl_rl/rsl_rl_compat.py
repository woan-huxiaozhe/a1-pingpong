...
"""Vendored copy of IsaacLab's ``isaaclab_rl.rsl_rl.utils.handle_deprecated_rsl_rl_cfg``.

Why this file exists
--------------------
The repo's PPO cfgs use the legacy ``policy=RslRlPpoActorCriticCfg(...)`` schema (deprecated
since rsl-rl 4.0). ``scripts/rsl_rl/train.py`` migrates them at runtime with
``handle_deprecated_rsl_rl_cfg()``.

That helper ships with the IsaacLab build used on the dev machine (Isaac Sim 4.5 / IsaacLab
2.3.2), but it is **absent** from the IsaacLab bundled with Isaac Sim 5.1 (the training box),
so ``from isaaclab_rl.rsl_rl import handle_deprecated_rsl_rl_cfg`` raises ``ImportError`` there.

``train.py`` therefore imports the helper from IsaacLab when present and falls back to this
copy otherwise -- keeping the cfgs in their validated ``policy=`` form (no hand-migration, which
would hit ``TypeError: MLPModel.__init__() got an unexpected keyword argument 'stochastic'``)
while running the identical migration logic on either IsaacLab version.

The cfg classes (``RslRlMLPModelCfg`` etc.) are imported dynamically *inside* the function, so
they resolve against whatever ``isaaclab_rl`` is installed. This module itself pulls in no
``pxr``/USD dependency and is safe to import before the Isaac Sim app launches.

Kept byte-for-byte equivalent to the IsaacLab source so behaviour does not drift. If IsaacLab's
upstream logic changes, re-sync this file.
"""
