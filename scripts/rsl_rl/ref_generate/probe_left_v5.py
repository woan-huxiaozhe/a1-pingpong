"""LEFT v5: NEGATIVE yb_3 shift to push paddle toward +Y and raise Z.

Finding from v4: yb_3+ → paddle -Y, -Z. So yb_3- → paddle +Y, +Z.
Data: each -0.15 in yb_3 ≈ +0.03 paddle Y, +0.035 paddle Z (extrapolated).

Target: paddle Y from +0.004 to +0.07 (Δ+0.066) and Z from 1.00 to 1.08 (Δ+0.08).
Estimated yb_3 shift needed: about -0.30 to -0.35.

MIDDLE yb_3 at hit window ≈ -1.85 to -2.00.
Shift by -0.30: → -2.15 to -2.30 (within limit -2.777).

Usage:
  /isaac-sim/python.sh scripts/rsl_rl/probe_left_v5.py --task X1-TableTennis
"""
...
def make_variant(yb3_shift=0.0, yb2_shift=0.0, yb1_shift=0.0):
...
VARIANTS = {
    ...
    "A": ("yb3-0.10", make_variant(yb3_shift=-0.10)),
    "B": ("yb3-0.20", make_variant(yb3_shift=-0.20)),
    "C": ("yb3-0.30", make_variant(yb3_shift=-0.30)),
    "D": ("yb3-0.40", make_variant(yb3_shift=-0.40)),
    "E": ("yb3-0.50", make_variant(yb3_shift=-0.50)),
    "F": ("yb3-0.30 yb2+0.05", make_variant(yb3_shift=-0.30, yb2_shift=+0.05)),
    "G": ("yb3-0.40 yb2+0.05", make_variant(yb3_shift=-0.40, yb2_shift=+0.05)),
    "H": ("yb3-0.30 yb1-0.10", make_variant(yb3_shift=-0.30, yb1_shift=-0.10)),
    "I": ("yb3-0.40 yb1-0.10", make_variant(yb3_shift=-0.40, yb1_shift=-0.10)),
    "J": ("yb3-0.50 yb1-0.10", make_variant(yb3_shift=-0.50, yb1_shift=-0.10)),
}
...
def main():
    ...
    print(f"\n{'='*130}")
    print(f"  LEFT v5: NEGATIVE yb_3 shift (push paddle toward +Y, raise Z)")
    print(f"  Ball target: X≈1.11, Y≈+0.07, Z≈1.08")
    print(f"{'='*130}")
    print(f"\n{'ID':<5} {'desc':<22} {'gap':>5} {'t':>5} {'HIT':>3} | "
    ...
    for vid in ["BASE", "A", "B", "C", "D", "E", "F", "G", "H", "I", "J"]:
        ...
        print(f" {vid:<4} {desc:<22} {mg:>5.3f} {mt:>5.3f} {hit_s:>3} | "
              f"{pp[0]:>+6.3f} {pp[1]:>+6.3f} {pp[2]:>+6.3f} | "
              f"{bp[1]:>+5.3f} {bp[2]:>+5.3f} | "
