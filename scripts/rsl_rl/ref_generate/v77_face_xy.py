"""V77: V71 vx=-1.65 太小, 用户要更大 vx 同时保持过网.

诊断:
  V71 face_n = (-0.02, -0.75, +0.66) — face Y 分量 0.75 "浪费" (球反射方向)
  V71 vx=-1.65, vz=+3.69 → 过网 (z@net=+1.07) 但 vx 弱

策略:
  1. yb_5 (wrist_roll) 调整 — 让 face Y 减小, X+Z 同时变大
  2. yb_6 PIN 测中间值 (-0.500, -0.600) 配 yb_5 调整
  3. yb_4 snap (elbow extend) 独立加 paddle vz, 不依赖 face 几何
  4. yb_1 windup 加深 + PIN 抬高加大 wu→PIN 的 ω

V71 base = V66 + yb_1 (wu -0.05, PIN +0.05) + yb_6 (wu -1.150, PIN -0.700)
"""
...
# V71 = 当前 npz (V66 + yb_1 V70 + yb_6 wu -1.150 PIN -0.700)
V71 = [
    ...
    (0.475, [+1.337, +0.103, -1.979, +0.457, -0.650, -0.700,  +1.000]),
...
def main():
    ...
    # ===== A: yb_5 PIN 调整 (减 face Y 分量, X+Z 自然变大) =====
    A1 = edits(V71, [(0.475, 4, -0.500)])  # less roll
    A2 = edits(V71, [(0.475, 4, -0.400)])
    A3 = edits(V71, [(0.475, 4, -0.300)])
    A4 = edits(V71, [(0.475, 4, -0.800)])  # more roll (反向)
    A5 = edits(V71, [(0.475, 4, -1.000)])

    # ===== B: yb_5 PIN + windup 同步 =====
    B1 = edits(V71, [(0.400, 4, -0.200), (0.475, 4, -0.400)])
    B2 = edits(V71, [(0.400, 4, -0.150), (0.475, 4, -0.300)])

    # ===== C: yb_6 PIN -0.500 (中间值) + yb_5 调整 =====
    C1 = edits(V71, [(0.475, 5, -0.500), (0.475, 4, -0.500)])
    C2 = edits(V71, [(0.475, 5, -0.500), (0.475, 4, -0.400)])
    C3 = edits(V71, [(0.475, 5, -0.500), (0.475, 4, -0.300)])
    C4 = edits(V71, [(0.475, 5, -0.600), (0.475, 4, -0.400)])

    # ===== D: yb_4 snap (elbow extension @ snap, 独立加 paddle vz) =====
    # V71 yb_4 t=0.475 PIN +0.457 → t=0.550 snap +0.407 (only +0.05 减少, 几乎无 ω)
    # 改 snap +0.700 (向回屈) → PIN→snap Δ=+0.243 → 前臂上摆给 +Z
    D1 = edits(V71, [(0.550, 3, +0.700)])  # snap 屈肘 +Z
    D2 = edits(V71, [(0.550, 3, +0.800)])
    D3 = edits(V71, [(0.475, 3, +0.300), (0.550, 3, +0.700)])  # PIN 早伸 + snap 屈

    # ===== E: yb_6 PIN -0.500 + yb_4 snap (face X + paddle vz 双补) =====
    E1 = edits(V71, [(0.475, 5, -0.500), (0.550, 3, +0.700)])
    E2 = edits(V71, [(0.475, 5, -0.500), (0.550, 3, +0.800)])
    E3 = edits(V71, [(0.475, 5, -0.500), (0.475, 3, +0.300), (0.550, 3, +0.700)])

    # ===== F: yb_6 PIN -0.600 + yb_4 snap (折中) =====
    F1 = edits(V71, [(0.475, 5, -0.600), (0.550, 3, +0.700)])
    F2 = edits(V71, [(0.475, 5, -0.600), (0.550, 3, +0.800)])

    # ===== G: 更激进 yb_1 (shoulder_pitch) windup + PIN snap =====
    # V71 yb_1: 0.300 t→1.127, 0.400→1.137, 0.475→1.337, 0.550→1.437. Δ(wu→PIN)=0.20.
    # 加深 windup -0.10 + 抬 PIN +0.05 = Δ 0.35 (1.75x)
    G1 = edits(V71, [(0.400, 0, +1.037), (0.475, 0, +1.387)])  # wu -0.10 PIN +0.05
    G2 = edits(V71, [(0.400, 0, +1.037), (0.475, 0, +1.437), (0.550, 0, +1.500)])  # 更激进
    G3 = edits(V71, [(0.300, 0, +1.080), (0.400, 0, +1.037), (0.475, 0, +1.387)])  # mid 也压

    probes = [
        ("V71 (current npz)", V71),
        ("A1: yb_5 PIN -0.500", A1),
        ("A2: yb_5 PIN -0.400", A2),
        ("A3: yb_5 PIN -0.300", A3),
        ("A4: yb_5 PIN -0.800", A4),
        ("A5: yb_5 PIN -1.000", A5),
        ("B1: yb_5 wu -0.20 PIN -0.40", B1),
        ("B2: yb_5 wu -0.15 PIN -0.30", B2),
        ("C1: yb_6-0.5 + yb_5-0.5", C1),
        ("C2: yb_6-0.5 + yb_5-0.4", C2),
        ("C3: yb_6-0.5 + yb_5-0.3", C3),
        ("C4: yb_6-0.6 + yb_5-0.4", C4),
        ("D1: yb_4 snap +0.700", D1),
        ("D2: yb_4 snap +0.800", D2),
        ("D3: yb_4 PIN+0.30 snap+0.70", D3),
        ("E1: yb_6-0.5 + yb_4 snap +0.70", E1),
        ("E2: yb_6-0.5 + yb_4 snap +0.80", E2),
        ("E3: yb_6-0.5 + yb_4 PIN+0.3 snap+0.7", E3),
        ("F1: yb_6-0.6 + yb_4 snap +0.70", F1),
        ("F2: yb_6-0.6 + yb_4 snap +0.80", F2),
        ("G1: yb_1 wu-0.10 PIN+0.05", G1),
        ("G2: yb_1 wu-0.10 PIN+0.10 snap+0.06", G2),
        ("G3: yb_1 mid-0.05 wu-0.10 PIN+0.05", G3),
    ]
    ...
    print("\n=== Sorted by ball post |vx| (核心: 越大越好) ===")
    ...
    for label, r in rows:
        ...
        print(f"  vx={bv[0]:+.2f}  bv=({bv[0]:+.2f},{bv[1]:+.2f},{bv[2]:+.2f})  "
              f"face_x={fn[0]:+.2f}  z@net={r['z_at_net']:+.2f}  x_bnc={xb_s}  {flag}  {label}")

    print("\n=== CLEARS net AND VALID (落对方台) ===")
    valid = [r for r in rows if r[1]["clears"] and r[1]["valid"]]
    if not valid:
        print("  (none)")
    valid.sort(key=lambda r: r[1]["ball_vel_post"][0])  # 最大 |vx| 优先
    for label, r in valid:
        bv = r["ball_vel_post"]
        print(f"  vx={bv[0]:+.2f}  z@net={r['z_at_net']:+.2f}  x_bnc={r['x_bounce']:+.2f}  "
