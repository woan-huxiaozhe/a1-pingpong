"""V82: V74 (F4) ref play 2/5 — 好的极好, 差的过不了网. 鲁棒性差.

V73: 3/4 ref play (vx -2.03, z@net +1.05)
V74: 2/5 ref play (vx -2.26, z@net +1.40 single phase)

V74 = V73 + yb_1(wu-0.05, PIN+0.05) + yb_3(PIN-1.850) + yb_5(PIN-0.900)

策略: 减一 + 半量 + 跨 phase 鲁棒性测试. 找鲁棒 + vx 涨的折中.
"""
...
def main():
    ...
    # V74 (full F4)
    V74 = edits(V73, [(0.400, 0, +1.087), (0.475, 0, +1.387),
                      (0.475, 2, -1.850), (0.475, 4, -0.900)])

    # 减一系列
    V74_no_yb1 = edits(V73, [(0.475, 2, -1.850), (0.475, 4, -0.900)])
    V74_no_yb3 = edits(V73, [(0.400, 0, +1.087), (0.475, 0, +1.387), (0.475, 4, -0.900)])
    V74_no_yb5 = edits(V73, [(0.400, 0, +1.087), (0.475, 0, +1.387), (0.475, 2, -1.850)])

    # 单一系列
    V73_yb1 = edits(V73, [(0.400, 0, +1.087), (0.475, 0, +1.387)])
    V73_yb3 = edits(V73, [(0.475, 2, -1.850)])
    V73_yb5 = edits(V73, [(0.475, 4, -0.900)])

    # 半量系列 (温和)
    V74_half = edits(V73, [(0.400, 0, +1.112), (0.475, 0, +1.362),  # yb_1 ±0.025
                           (0.475, 2, -1.915),                       # yb_3 -1.915
                           (0.475, 4, -0.850)])                       # yb_5 -0.850
    V74_yb5_half = edits(V73, [(0.400, 0, +1.087), (0.475, 0, +1.387),
                               (0.475, 2, -1.850), (0.475, 4, -0.850)])  # 仅 yb_5 折中

    probes = [
        ("V73 baseline (3/4)", V73),
        ("V74 full (F4)", V74),
        ("V74 - yb_1 (=yb_3+yb_5)", V74_no_yb1),
        ("V74 - yb_3 (=yb_1+yb_5)", V74_no_yb3),
        ("V74 - yb_5 (=yb_1+yb_3)", V74_no_yb5),
        ("V73 + yb_1 only (D1)", V73_yb1),
        ("V73 + yb_3 only (C1)", V73_yb3),
        ("V73 + yb_5 only (A1)", V73_yb5),
        ("V74 半量 (yb_1 ±0.025, yb_3 -1.915, yb_5 -0.850)", V74_half),
        ("V74 yb_5 折中 -0.850", V74_yb5_half),
    ]
    ...
    print("\n=== Sorted by CLEARS, then min_zn (鲁棒性优先) ===")
    rows.sort(key=lambda r: (-r[1], -r[3]))
    for label, clears, mean_zn, min_zn, mean_vx, _ in rows:
        print(f"  CLR {clears}/5  vx={mean_vx:+.2f}  zn[mean/min]={mean_zn:+.2f}/{min_zn:+.2f}  {label}")

    print("\n=== Sorted by mean_vx (力度优先) ===")
    rows.sort(key=lambda r: r[4])
    for label, clears, mean_zn, min_zn, mean_vx, _ in rows[:5]:
        print(f"  vx={mean_vx:+.2f}  CLR {clears}/5  zn[mean/min]={mean_zn:+.2f}/{min_zn:+.2f}  {label}")
