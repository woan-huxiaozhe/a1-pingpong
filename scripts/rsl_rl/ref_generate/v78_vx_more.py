"""V78: V72 vx=-2.02, 用户要更快. v77 yb_5 单点 -0.800 是 local max,
v77 没试 yb_5 hit-window 整体加深 + yb_5/yb_6 联调 + yb_3 调.

V72 base = V66 + yb_1(wu-0.05, PIN+0.05) + yb_6(wu-1.150, PIN-0.700) + yb_5(PIN-0.800)
"""
...
def main():
    ...
    # ===== A: yb_5 hit-window 整体加深 (mid+wu+PIN 一起加 -0.15) =====
    A1 = edits(V72, [(0.300, 4, -0.465), (0.400, 4, -0.465)])  # mid + wu 一起 -0.15 (PIN 已 -0.800)
    A2 = edits(V72, [(0.300, 4, -0.515), (0.400, 4, -0.515), (0.475, 4, -0.850)])
    A3 = edits(V72, [(0.300, 4, -0.465), (0.400, 4, -0.465), (0.550, 4, -0.300)])  # snap 也加深

    # ===== B: yb_5 PIN fine-tune around -0.800 =====
    B1 = edits(V72, [(0.475, 4, -0.750)])
    B2 = edits(V72, [(0.475, 4, -0.850)])
    B3 = edits(V72, [(0.475, 4, -0.900)])
    B4 = edits(V72, [(0.475, 4, -0.700)])

    # ===== C: yb_6 PIN raise (face X 增加) on top of V72 yb_5=-0.800 =====
    # v77 C 系列错把 yb_5 调成 less negative, 现在 yb_5 -0.800 配合 yb_6 PIN -0.500/-0.300
    C1 = edits(V72, [(0.475, 5, -0.500)])
    C2 = edits(V72, [(0.475, 5, -0.400)])
    C3 = edits(V72, [(0.475, 5, -0.300)])  # V67 历史值
    C4 = edits(V72, [(0.475, 5, -0.500), (0.400, 5, -1.050)])  # PIN 抬时 windup 同步放松

    # ===== D: yb_3 (shoulder_yaw) less negative — 整体扫拍角度向 -X =====
    # V72 yb_3 PIN -1.979. less negative 让肩外送 (理论上 paddle vx 增加)
    D1 = edits(V72, [(0.475, 2, -1.850)])
    D2 = edits(V72, [(0.475, 2, -1.700)])
    D3 = edits(V72, [(0.475, 2, -1.850), (0.400, 2, -1.850), (0.300, 2, -1.850)])  # hw 整体

    # ===== E: yb_6 windup 加深 (PIN→snap 的反向 ω 在 hit 时刻贡献 +Z, 但 windup→PIN 的 ω 贡献 -X) =====
    # V72 yb_6 wu -1.150 PIN -0.700 → Δ=0.450. 加深 windup 到 -1.250, Δ=0.550 (+22%)
    E1 = edits(V72, [(0.400, 5, -1.200)])
    E2 = edits(V72, [(0.400, 5, -1.250)])
    E3 = edits(V72, [(0.400, 5, -1.270)])  # 留 18 mrad spline buffer

    # ===== F: 大组合 — yb_5 + yb_6 + yb_3 同时优化 =====
    F1 = edits(V72, [(0.475, 5, -0.500), (0.475, 2, -1.850)])  # yb_6 抬 + yb_3 less neg
    F2 = edits(V72, [(0.475, 5, -0.500), (0.300, 4, -0.465), (0.400, 4, -0.465)])  # yb_5 hw + yb_6 抬
    F3 = edits(V72, [(0.475, 5, -0.500), (0.400, 5, -1.050), (0.475, 2, -1.850)])  # 安全 yb_6 抬 + yb_3
    F4 = edits(V72, [(0.400, 5, -1.250), (0.475, 5, -0.500)])  # windup 深 + PIN 抬

    # ===== G: yb_1 (shoulder_pitch) 加大 windup→PIN ω, 推球 -X =====
    G1 = edits(V72, [(0.400, 0, +1.087), (0.475, 0, +1.387)])  # wu -0.05 PIN +0.05
    G2 = edits(V72, [(0.400, 0, +1.037), (0.475, 0, +1.437), (0.550, 0, +1.500)])  # 大幅
    G3 = edits(V72, [(0.300, 0, +1.080), (0.400, 0, +1.087)])  # mid+wu 压, PIN 不动

    probes = [
        ("V72 (current npz)", V72),
        ("A1: yb_5 mid/wu 加深 -0.465", A1),
        ("A2: yb_5 mid/wu/PIN 全深 -0.515/-0.515/-0.850", A2),
        ("A3: yb_5 mid/wu/snap 全 -0.15", A3),
        ("B1: yb_5 PIN -0.750", B1),
        ("B2: yb_5 PIN -0.850", B2),
        ("B3: yb_5 PIN -0.900", B3),
        ("B4: yb_5 PIN -0.700", B4),
        ("C1: yb_6 PIN -0.500", C1),
        ("C2: yb_6 PIN -0.400", C2),
        ("C3: yb_6 PIN -0.300", C3),
        ("C4: yb_6 wu-1.05 PIN-0.50", C4),
        ("D1: yb_3 PIN -1.850", D1),
        ("D2: yb_3 PIN -1.700", D2),
        ("D3: yb_3 hw -1.850", D3),
        ("E1: yb_6 wu -1.200", E1),
        ("E2: yb_6 wu -1.250", E2),
        ("E3: yb_6 wu -1.270 (limit-18)", E3),
        ("F1: yb_6-0.5 + yb_3-1.85", F1),
        ("F2: yb_5 hw + yb_6-0.5", F2),
        ("F3: yb_6 wu/PIN 安全 + yb_3", F3),
        ("F4: yb_6 wu-1.25 + PIN-0.5", F4),
        ("G1: yb_1 wu-0.05 PIN+0.05", G1),
        ("G2: yb_1 wu-0.10 PIN+0.10 snap+0.06", G2),
        ("G3: yb_1 mid-0.05 wu-0.05", G3),
    ]
